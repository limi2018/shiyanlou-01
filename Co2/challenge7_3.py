...
def data_clean():
    ...
    df_ghg = pd.DataFrame(df_data).loc[df_data['Series code'].isin(['EN.ATM.CO2E.KT', 'EN.ATM.METH.KT.CE', 'EN.ATM.NOXE.KT.CE', 'EN.ATM.GHGO.KT.CE', 'EN.CLC.GHGR.MT.CE'])]
    ...
    df_gt_reindex = pd.DataFrame(df_gt.iloc[:, [1,4]].values, index=time_index, columns=[['Land Average Temperature','Land And Ocean Average Temperature']])
    ...
    df_merge = pd.DataFrame(df_merge_temp.iloc[:,[1,2,4]].values,index=df_merge_temp['index'],columns=['Land Average Temperature','Land And Ocean Average Temperature', 'Total GHG'])
...
def climate_plot():
    ...
    ax1 = df_merge_max_min.plot(kind='line',figsize=(16,9),ax=axes[0,0],)
    ax1.set_xlabel('Years')
    ax1.set_ylabel('Values')
    
    ax2 = df_merge_max_min.plot( kind='bar',figsize=(16, 9),ax=axes[0, 1],)
    ax2.set_xlabel('Years')
    ax2.set_ylabel('Values')
    
    ax3 = df_gt_resample_Q.plot(kind='area',figsize=(16, 9),ax=axes[1, 0], )
    ax3.set_xlabel('Quarters')
    ax3.set_ylabel('Temperature')
    
    ax4 = df_gt_resample_Q.plot(kind='kde',figsize=(16, 9),ax=axes[1, 1],)
